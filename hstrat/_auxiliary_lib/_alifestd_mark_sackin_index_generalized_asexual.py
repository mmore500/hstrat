...
def alifestd_mark_sackin_index_generalized_asexual(
    ...
    """Add column `sackin_index` with generalized Sackin index for each
    subtree.

    Computes a generalized Sackin imbalance index that supports polytomies.
    The Sackin index is the sum of the depths of all leaves in the subtree.
    For each internal node, the contribution is the sum of leaf depths in its
    subtree.

    For a node with children c_1, c_2, ..., c_k:
        sackin[node] = sum_{i} (sackin[c_i] + num_leaves[c_i])

    For strictly bifurcating trees, this is equivalent to the classic Sackin
    index. For trees known to be strictly bifurcating, consider using
    `alifestd_mark_sackin_index_asexual` for consistency.
    ...
    pd.DataFrame
        ...
        containing the generalized Sackin imbalance index for the subtree
        rooted at each node.

    See Also
    --------
    alifestd_mark_sackin_index_asexual :
        Classic Sackin index for strictly bifurcating trees.
    """
    if not mutate:
    ...
    if alifestd_has_contiguous_ids(phylogeny_df):
        ...
        phylogeny_df[
            "sackin_index"
        ] = alifestd_mark_sackin_index_generalized_asexual_fast_path(
