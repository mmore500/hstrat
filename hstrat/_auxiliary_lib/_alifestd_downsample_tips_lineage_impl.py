...
def _alifestd_downsample_tips_lineage_impl(
    ...
    num_tips: int,
    calc_mrca_vector: typing.Callable,
) -> np.ndarray:
    """Shared numpy implementation for lineage-based tip downsampling.

    Selects the target leaf (largest target value, ties broken by global
    numpy RNG), computes off-lineage deltas via a caller-supplied MRCA
    vector function, and returns a boolean extant mask.

    Callers should wrap this function with ``with_rng_state_context``
    when deterministic behaviour is required.
    ...
    calc_mrca_vector : callable
        ``calc_mrca_vector(target_id)`` returns a numpy int array of
        MRCA ids for each taxon with respect to *target_id*.
        Taxa in a different tree should receive ``-1``.
    ...
    # Select target leaf (largest criterion_target; ties broken by RNG)
    max_target = leaf_target_values.max()
    candidate_ids = leaf_ids[leaf_target_values == max_target]
    target_id = int(np.random.choice(candidate_ids))

    # Compute MRCA vector (framework-specific)
    mrca_vector = calc_mrca_vector(target_id)
