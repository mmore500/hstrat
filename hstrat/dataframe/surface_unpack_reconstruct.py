...
description:
  ...
  End-users are recommended to prefer `hstrat.dataframe.build_tree_surface`.
  Reads raw genome data from tabular data file(s) and writes reconstructed
  phylogeny data to output file in alife standard format. Surface-only
  implementation; Not compatible with HereditaryStratigraphicColumn genome
  annotations. Uses naive method to estimate taxon `origin_time`s and performs
  ...
  'dstream_S' : integer
      ...
      i.e., num differentia stored per annotation
  ...
  For these columns, output rows for tip nodes are assigned values from
  corresponding genome row in original data. Internal tree nodes will take
  null values in user-provided columns.
  ...
  Behind the scenes, the following processing steps occur:
   1. Data is loaded from user-provided tabular data file(s), with
      format inferred from filename extension (if not explicitly provided).
   2. Any data transformations from CLI flags and arguments are applied.
   3. Annotation data (i.e., downstream buffer and counter components
      of hstrat surface) are extracted from raw genome fields. For details,
      see `python3 -m downstream.dataframe.unpack_data_packed --help`.
   4. Differentia are unpacked from downstream buffer and data item
      origin times are decoded from generation counter and buffer size.
      For details, see `python3 -m downstream.dataframe.explode_lookup_unpacked --help`
   5. A trie-based algorithm is used to reconstruct an estimate of
      phylogenetic history from annotation data.
   6. Reconstructed phylogeny data is joined with user-provided columns
      from original data files.
   7. Phylogeny data is saved in alife standard format to user-specified
      output file, with file type inferred from filename extension
      (if not explicitly provided).

  To streamline memory and disk usage, consider using CLI flags to cast
  string columns with repeated data values to categorical, shrink data
  types, or drop superfluous columns. Dataframe operations are conducted
  using polars and downstream operations may employ numba, both of which
  are capable of thread-based parallelism. Environment variables
  POLARS_MAX_THREADS and NUMBA_NUM_THREADS may be used to tune thread usage.
"""
