...
class CalcMrcaUncertaintyAbsExact:
    ...
    def __call__(
        ...
        elif least_last_rank < spec.GetFixedResolution():
            return least_last_rank - 1
        # mrca between last regularly-spaced rank and tail rank
        elif actual_rank_of_mrca >= (
            least_last_rank - least_last_rank % spec.GetFixedResolution()
        ):
            return (least_last_rank - 1) % spec.GetFixedResolution()
        # mrca between two regularly-spaced ranks
        else:
            return spec.GetFixedResolution() - 1
