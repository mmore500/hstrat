...
def test_policy_consistency(fixed_resolution, time_sequence):
    ...
    spec = policy.GetSpec()
    instance = fixed_resolution_policy.CalcRankAtColumnIndex(spec)
    for num_strata_deposited in time_sequence:
        for which in (
            instance,
            fixed_resolution_policy.CalcRankAtColumnIndex(spec),
        ):
            assert all(
...
def test_eq(fixed_resolution):
    ...
    assert instance == fixed_resolution_policy.CalcRankAtColumnIndex(spec)
