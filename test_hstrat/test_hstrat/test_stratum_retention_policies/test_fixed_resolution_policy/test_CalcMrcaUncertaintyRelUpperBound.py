...
from hstrat.hstrat import fixed_resolution_policy
...
def test_policy_consistency(fixed_resolution, time_sequence):
    ...
    spec = policy.GetSpec()
    instance = fixed_resolution_policy.CalcMrcaUncertaintyRelUpperBound(
        spec,
    )
    for num_strata_deposited in time_sequence:
        for actual_mrca_rank in (
            ...
            for which in (
                instance,
                fixed_resolution_policy.CalcMrcaUncertaintyRelUpperBound(spec),
            ):
                assert (
                    which(
                        policy,
...
def test_eq(fixed_resolution):
    policy = fixed_resolution_policy.Policy(fixed_resolution)
    spec = policy.GetSpec()
    instance = fixed_resolution_policy.CalcMrcaUncertaintyRelUpperBound(spec)

    assert instance == instance
    assert (
        instance
        == fixed_resolution_policy.CalcMrcaUncertaintyRelUpperBound(
            spec,
        )
    )
