...
from hstrat.hstrat import geom_seq_nth_root_tapered_policy
from hstrat.hstrat.stratum_retention_policies._detail import (
    ...
    for num_strata_deposited_a in time_sequence:
        for num_strata_deposited_b in (
            ...
            for actual_mrca_rank in np.random.default_rng(1).integers(
                ...
                for which in (
                    instance,
                    geom_seq_nth_root_tapered_policy.CalcMrcaUncertaintyAbsUpperBoundAtPessimalRank(
                        spec
                    ),
                ):
                    assert (
