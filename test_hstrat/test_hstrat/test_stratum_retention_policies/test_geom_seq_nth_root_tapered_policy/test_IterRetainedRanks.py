...
from hstrat.helpers import pairwise
...
def test_only_dwindling_over_time(degree, interspersal, time_sequence):
    ...
    spec = policy.GetSpec()
    instance = geom_seq_nth_root_tapered_policy.IterRetainedRanks(spec)
    for num_strata_deposited in time_sequence:
        for which in (
            instance,
            geom_seq_nth_root_tapered_policy.IterRetainedRanks(spec),
        ):
            cur_set = {
                *which(
                    policy,
...
def test_eq(degree, interspersal):
    ...
    assert instance == geom_seq_nth_root_tapered_policy.IterRetainedRanks(spec)
