...
def test_init(degree, interspersal):
    assert (
        ...
        == geom_seq_nth_root_tapered_policy.Policy(
            policy_spec=geom_seq_nth_root_tapered_policy.PolicySpec(
                degree, interspersal
            ),
        ).GetSpec()
    )
...
def test_eq(degree, interspersal):
    ...
    assert policy == geom_seq_nth_root_tapered_policy.Policy(
        degree, interspersal
    )
    ...
    assert not policy == geom_seq_nth_root_tapered_policy.Policy(
...
def test_GetSpec(degree, interspersal):
    assert geom_seq_nth_root_tapered_policy.Policy(
        degree, interspersal
    ).GetSpec() == geom_seq_nth_root_tapered_policy.PolicySpec(
        degree, interspersal
    )
