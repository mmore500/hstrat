...
def test_policy_consistency(time_sequence):
    ...
    spec = policy.GetSpec()
    instance = perfect_resolution_policy.CalcRankAtColumnIndex(spec)
    for num_strata_deposited in time_sequence:
        for which in (
            instance,
            perfect_resolution_policy.CalcRankAtColumnIndex(spec),
        ):
            assert all(
...
def test_eq():
    ...
    assert instance == perfect_resolution_policy.CalcRankAtColumnIndex(spec)
