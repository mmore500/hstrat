...
from hstrat.hstrat import depth_proportional_resolution_tapered_policy
...
def test_init(depth_proportional_resolution):
    assert (
        ...
        == depth_proportional_resolution_tapered_policy.Policy(
            policy_spec=depth_proportional_resolution_tapered_policy.PolicySpec(
                depth_proportional_resolution
            ),
        ).GetSpec()
    )
...
def test_eq(depth_proportional_resolution):
    ...
    assert policy == depth_proportional_resolution_tapered_policy.Policy(
        depth_proportional_resolution
    )
    ...
    assert not policy == depth_proportional_resolution_tapered_policy.Policy(
...
def test_GetSpec(depth_proportional_resolution):
    assert depth_proportional_resolution_tapered_policy.Policy(
        depth_proportional_resolution
    ).GetSpec() == depth_proportional_resolution_tapered_policy.PolicySpec(
        depth_proportional_resolution
    )
...
def test_WithoutCalcRankAtColumnIndex(depth_proportional_resolution):
    ...
    original = depth_proportional_resolution_tapered_policy.Policy(
        depth_proportional_resolution
    )
