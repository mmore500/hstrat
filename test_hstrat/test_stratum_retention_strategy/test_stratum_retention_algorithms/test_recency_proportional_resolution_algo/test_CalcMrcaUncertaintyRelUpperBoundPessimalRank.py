...
def test_policy_consistency(recency_proportional_resolution, time_sequence):
    ...
    spec = policy.GetSpec()
    instance = recency_proportional_resolution_algo.CalcMrcaUncertaintyRelUpperBoundPessimalRank(
        spec,
    )
    for num_strata_deposited_a in time_sequence:
        for num_strata_deposited_b in (
            ...
            for actual_mrca_rank in np.random.default_rng(1).integers(
                ...
                for which in (
                    instance,
                    recency_proportional_resolution_algo.CalcMrcaUncertaintyRelUpperBoundPessimalRank(
                        spec
                    ),
                ):
                    assert (
