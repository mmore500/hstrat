...
def test_policy_consistency(degree, interspersal, time_sequence):
    ...
    spec = policy.GetSpec()
    instance = geom_seq_nth_root_tapered_algo.CalcNumStrataRetainedExact(spec)
    for num_strata_deposited in time_sequence:
        ...
        for which in (
            instance,
            geom_seq_nth_root_tapered_algo.CalcNumStrataRetainedExact(spec),
        ):
            assert (
                which(
                    policy,
...
def test_eq(degree, interspersal):
    ...
    assert (
        instance
        == geom_seq_nth_root_tapered_algo.CalcNumStrataRetainedExact(
            spec,
        )
    )
