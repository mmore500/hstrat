...
def test_policy_consistency(time_sequence):
    ...
    spec = policy.GetSpec()
    instance = nominal_resolution_algo.CalcRankAtColumnIndex(spec)
    for num_strata_deposited in time_sequence:
        for which in (
            instance,
            nominal_resolution_algo.CalcRankAtColumnIndex(spec),
        ):
            assert all(
...
def test_eq():
    ...
    assert instance == nominal_resolution_algo.CalcRankAtColumnIndex(spec)
