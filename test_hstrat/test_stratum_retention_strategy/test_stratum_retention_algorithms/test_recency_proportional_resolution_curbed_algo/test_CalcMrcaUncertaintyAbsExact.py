...
def test_policy_consistency(size_curb, time_sequence):
    ...
    spec = policy.GetSpec()
    instance = recency_proportional_resolution_curbed_algo.CalcMrcaUncertaintyAbsExact(
        spec
    )
    for num_strata_deposited in time_sequence:
        ...
        for actual_mrca_rank in it.chain(
            ...
            for which in (
                instance,
                recency_proportional_resolution_curbed_algo.CalcMrcaUncertaintyAbsExact(
                    spec
                ),
            ):
                assert (
