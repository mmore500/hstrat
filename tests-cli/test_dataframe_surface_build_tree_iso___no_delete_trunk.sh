...
ls -1 "${genomes}" \
    ...
    ${HSTRAT_TESTS_CLI_HEAD:-} --collapse-unif-freq=0 --no-delete-trunk \
    > ${HSTRAT_TESTS_CLI_STDOUT} 2>&1
...
for opt in \
    ...
    "--exploded-slice-size=4_000_000 --collapse-unif-freq=0 --no-delete-trunk" \
    "--exploded-slice-size=1_000 --collapse-unif-freq=0 --no-delete-trunk" \
    "--no-delete-trunk" \
    "--exploded-slice-size=1_000_000 --no-delete-trunk" \
    "--exploded-slice-size=4_000_000 --no-delete-trunk" \
; do
    echo "   - opt=${opt}"
