...
def test_policy_consistency(recency_proportional_resolution, time_sequence):
    ...
    spec = policy.GetSpec()
    instance = recency_proportional_resolution_policy.CalcMrcaUncertaintyRelUpperBoundAtPessimalRank(
        spec,
    )
    for num_strata_deposited_a in time_sequence:
        for num_strata_deposited_b in (
            ...
            for actual_mrca_rank in np.random.default_rng(1).integers(
                ...
                for which in (
                    instance,
                    recency_proportional_resolution_policy.CalcMrcaUncertaintyRelUpperBoundAtPessimalRank(spec)
                ):
                    assert which(
                        policy,
                        num_strata_deposited_a,
...
def test_eq(recency_proportional_resolution):
    policy = recency_proportional_resolution_policy.Policy(recency_proportional_resolution)
    spec = policy.GetSpec()
    instance = recency_proportional_resolution_policy.CalcMrcaUncertaintyRelUpperBoundAtPessimalRank(spec)

    assert instance == instance
    assert instance == recency_proportional_resolution_policy.CalcMrcaUncertaintyRelUpperBoundAtPessimalRank(
        spec,
    )
