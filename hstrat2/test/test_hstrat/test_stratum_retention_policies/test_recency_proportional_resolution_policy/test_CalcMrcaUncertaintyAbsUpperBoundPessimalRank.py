    ...
    for num_strata_deposited_a in time_sequence:
        for num_strata_deposited_b in (
            ...
            for actual_mrca_rank in np.random.default_rng(1).integers(
                ...
                for which in (
                    instance,
                    recency_proportional_resolution_policy.CalcMrcaUncertaintyAbsUpperBoundPessimalRank(spec)
                ):
...
def test_eq(recency_proportional_resolution):
    policy = recency_proportional_resolution_policy.Policy(recency_proportional_resolution)
    spec = policy.GetSpec()
    instance = recency_proportional_resolution_policy.CalcMrcaUncertaintyAbsUpperBoundPessimalRank(spec)

    assert instance == instance
    assert instance == recency_proportional_resolution_policy.CalcMrcaUncertaintyAbsUpperBoundPessimalRank(
        spec,
    )
