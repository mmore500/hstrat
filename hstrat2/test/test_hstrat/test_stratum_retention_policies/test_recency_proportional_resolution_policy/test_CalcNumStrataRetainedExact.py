...
def test_policy_consistency(recency_proportional_resolution, time_sequence):
    ...
    spec = policy.GetSpec()
    instance = recency_proportional_resolution_policy.CalcNumStrataRetainedExact(spec)
    for num_strata_deposited in time_sequence:
        ...
        for which in (
            instance,
            recency_proportional_resolution_policy.CalcNumStrataRetainedExact(spec),
        ):
            assert which(
                policy,
                num_strata_deposited,
...
def test_eq(recency_proportional_resolution):
    ...
    assert instance == recency_proportional_resolution_policy.CalcNumStrataRetainedExact(
        spec,
    )
