...
def test_policy_consistency(degree, interspersal, time_sequence):
    ...
    spec = policy.GetSpec()
    instance = geom_seq_nth_root_policy.CalcMrcaUncertaintyRelUpperBoundAtPessimalRank(
        spec,
    )
    for num_strata_deposited_a in time_sequence:
        for num_strata_deposited_b in (
            ...
            for actual_mrca_rank in np.random.default_rng(1).integers(
                ...
                for which in (
                    instance,
                    geom_seq_nth_root_policy.CalcMrcaUncertaintyRelUpperBoundAtPessimalRank(spec)
                ):
                    assert which(
                        policy,
                        num_strata_deposited_a,
...
def test_eq(degree, interspersal):
    policy = geom_seq_nth_root_policy.Policy(degree, interspersal)
    spec = policy.GetSpec()
    instance = geom_seq_nth_root_policy.CalcMrcaUncertaintyRelUpperBoundAtPessimalRank(spec)

    assert instance == instance
    assert instance == geom_seq_nth_root_policy.CalcMrcaUncertaintyRelUpperBoundAtPessimalRank(
        spec,
    )
