...
def test_policy_consistency(time_sequence):
    ...
    spec = policy.GetSpec()
    instance = perfect_resolution_policy.CalcMrcaUncertaintyRelUpperBoundPessimalRank(
        spec,
    )
    for num_strata_deposited_a in time_sequence:
        for num_strata_deposited_b in (
            ...
            for actual_mrca_rank in np.random.default_rng(1).integers(
                ...
                for which in (
                    instance,
                    perfect_resolution_policy.CalcMrcaUncertaintyRelUpperBoundPessimalRank(spec)
                ):
...
def test_eq():
    policy = perfect_resolution_policy.Policy()
    spec = policy.GetSpec()
    instance = perfect_resolution_policy.CalcMrcaUncertaintyRelUpperBoundPessimalRank(spec)

    assert instance == instance
    assert instance == perfect_resolution_policy.CalcMrcaUncertaintyRelUpperBoundPessimalRank(
        spec,
    )
