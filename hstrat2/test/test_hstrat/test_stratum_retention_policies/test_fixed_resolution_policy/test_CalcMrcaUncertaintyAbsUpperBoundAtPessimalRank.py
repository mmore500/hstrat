...
def test_policy_consistency(fixed_resolution, time_sequence):
    ...
    spec = policy.GetSpec()
    instance = fixed_resolution_policy.CalcMrcaUncertaintyAbsUpperBoundAtPessimalRank(
        spec,
    )
    for num_strata_deposited_a in time_sequence:
        for num_strata_deposited_b in (
            ...
            for actual_mrca_rank in np.random.default_rng(1).integers(
                ...
                for which in (
                    instance,
                    fixed_resolution_policy.CalcMrcaUncertaintyAbsUpperBoundAtPessimalRank(spec)
                ):
                    assert which(
                        policy,
                        num_strata_deposited_a,
...
def test_eq(fixed_resolution):
    policy = fixed_resolution_policy.Policy(fixed_resolution)
    spec = policy.GetSpec()
    instance = fixed_resolution_policy.CalcMrcaUncertaintyAbsUpperBoundAtPessimalRank(spec)

    assert instance == instance
    assert instance == fixed_resolution_policy.CalcMrcaUncertaintyAbsUpperBoundAtPessimalRank(
        spec,
    )
