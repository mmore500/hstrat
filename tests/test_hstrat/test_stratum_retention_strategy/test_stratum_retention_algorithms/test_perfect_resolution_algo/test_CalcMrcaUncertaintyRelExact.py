...
def test_policy_consistency(time_sequence):
    ...
    spec = policy.GetSpec()
    instance = perfect_resolution_algo.CalcMrcaUncertaintyRelExact(spec)
    for num_strata_deposited in time_sequence:
        ...
        for actual_mrca_rank in (
            ...
            policy_requirement = (ub - lb - 1) / recency
            assert policy_requirement >= 0
            for which in (
                instance,
                perfect_resolution_algo.CalcMrcaUncertaintyRelExact(spec),
            ):
                assert (
                    which(
                        policy,
...
def test_eq():
    ...
    assert instance == perfect_resolution_algo.CalcMrcaUncertaintyRelExact(
        spec
    )
