...
def test_policy_consistency(time_sequence):
    ...
    spec = policy.GetSpec()
    instance = nominal_resolution_algo.CalcMrcaUncertaintyAbsUpperBound(
        spec,
    )
    for num_strata_deposited in time_sequence:
        for actual_mrca_rank in (
            ...
            for which in (
                instance,
                nominal_resolution_algo.CalcMrcaUncertaintyAbsUpperBound(spec),
            ):
                assert (
                    which(
                        policy,
...
def test_eq():
    policy = nominal_resolution_algo.Policy()
    spec = policy.GetSpec()
    instance = nominal_resolution_algo.CalcMrcaUncertaintyAbsUpperBound(spec)

    assert instance == instance
    assert (
        instance
        == nominal_resolution_algo.CalcMrcaUncertaintyAbsUpperBound(
            spec,
        )
    )
