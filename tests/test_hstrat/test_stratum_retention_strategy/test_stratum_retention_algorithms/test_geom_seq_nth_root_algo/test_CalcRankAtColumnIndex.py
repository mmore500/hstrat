...
def test_policy_consistency(degree, interspersal, time_sequence):
    ...
    spec = policy.GetSpec()
    instance = geom_seq_nth_root_algo.CalcRankAtColumnIndex(spec)
    for num_strata_deposited in time_sequence:
        for which in (
            instance,
            geom_seq_nth_root_algo.CalcRankAtColumnIndex(spec),
        ):
            assert all(
...
def test_eq(degree, interspersal):
    ...
    assert instance == geom_seq_nth_root_algo.CalcRankAtColumnIndex(spec)
