...
def test_policy_consistency(size_curb, time_sequence):
    ...
    spec = policy.GetSpec()
    instance = recency_proportional_resolution_curbed_algo.CalcMrcaUncertaintyAbsUpperBoundPessimalRank(
        spec,
    )
    for num_strata_deposited_a in time_sequence:
        for num_strata_deposited_b in (
            ...
            for which in (
                instance,
                recency_proportional_resolution_curbed_algo.CalcMrcaUncertaintyAbsUpperBoundPessimalRank(
                    spec
                ),
            ):
                assert (
