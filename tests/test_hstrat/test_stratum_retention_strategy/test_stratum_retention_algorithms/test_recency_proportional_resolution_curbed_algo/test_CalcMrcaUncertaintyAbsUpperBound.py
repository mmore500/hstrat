...
def test_policy_consistency(size_curb, time_sequence):
    ...
    spec = policy.GetSpec()
    instance = recency_proportional_resolution_curbed_algo.CalcMrcaUncertaintyAbsUpperBound(
        spec,
    )
    for num_strata_deposited in time_sequence:
        for actual_mrca_rank in (
            ...
            for which in (
                instance,
                recency_proportional_resolution_curbed_algo.CalcMrcaUncertaintyAbsUpperBound(
                    spec
                ),
            ):
                assert (
