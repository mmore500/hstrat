...
def test_policy_consistency(replicate):
    ...
    spec = policy.GetSpec()
    instance = stochastic_algo.CalcMrcaUncertaintyAbsUpperBound(spec)
    for num_strata_deposited in range(1, 200):
        for actual_mrca_rank in it.chain(
            ...
            for which in (
                instance,
                stochastic_algo.CalcMrcaUncertaintyAbsUpperBound(spec),
            ):
                assert (
                    which(
                        policy,
...
def test_eq(replicate):
    ...
    assert instance == stochastic_algo.CalcMrcaUncertaintyAbsUpperBound(
        spec,
    )
