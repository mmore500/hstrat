...
def test_is_genesis_of_allele():
    ...
    assert inner_node.IsGenesisOfAllele(1, 2)
    assert not inner_node.IsGenesisOfAllele(1, 3)
    assert not inner_node.IsGenesisOfAllele(2, 2)

    assert not root_node.IsGenesisOfAllele(1, 2)


def test_find_geneses_of_allele_self(subtrie):
    geneses = [*subtrie.FindGenesesOfAllele(3, 1)]
    assert len(geneses) == 1
    assert geneses[0] is subtrie


def test_find_geneses_of_allele_singleton(subtrie):
    geneses = [*subtrie.FindGenesesOfAllele(6, 21)]
    assert len(geneses) == 1
    assert geneses[0].IsGenesisOfAllele(6, 21)


def test_find_geneses_of_allele_multiple(subtrie):
    geneses = [*subtrie.FindGenesesOfAllele(6, 19)]
    assert len(geneses) == 3

    for genesis in geneses:
        assert genesis.IsGenesisOfAllele(6, 19)

    geneses = [*subtrie.FindGenesesOfAllele(4, 17)]
    assert len(geneses) == 2

    for genesis in geneses:
        assert genesis.IsGenesisOfAllele(4, 17)

    geneses = [*subtrie.FindGenesesOfAllele(5, 11)]
    assert len(geneses) == 2

    for genesis in geneses:
        assert genesis.IsGenesisOfAllele(5, 11)


def test_find_geneses_of_allele_failure(subtrie):
    geneses = [*subtrie.FindGenesesOfAllele(3, 19)]
    assert len(geneses) == 0

    geneses = [*subtrie.FindGenesesOfAllele(3, 31)]
    assert len(geneses) == 0

    geneses = [*subtrie.FindGenesesOfAllele(11, 31)]
    assert len(geneses) == 0

    geneses = [*subtrie.FindGenesesOfAllele(5, 21)]
    assert len(geneses) == 0
...
def test_get_deepest_consecutive_shared_allele_genesis_with_empty_iterator(
    ...
    (
        genesis,
        leftover_iter,
    ) = subtrie.GetDeepestConsecutiveSharedAlleleGenesis(iter([]))
...
def test_get_deepest_consecutive_shared_allele_genesis_singleton(
    ...
    (
        genesis,
        leftover_iter,
    ) = subtrie.GetDeepestConsecutiveSharedAlleleGenesis(iter(alleles))
    assert genesis.IsGenesisOfAllele(5, 11)
    assert genesis.parent.IsGenesisOfAllele(4, 16)
    assert [*leftover_iter] == []
    ...
    assert genesis.IsGenesisOfAllele(4, 16)
    assert [*leftover_iter] == []
...
def test_insert_taxon_canopy1(subtrie):
    ...
    assert inserted.parent.IsGenesisOfAllele(4, 17)
    ...
    assert inserted.parent.IsGenesisOfAllele(9, 12)
...
def test_insert_taxon_canopy3(subtrie):
    ...
    assert inserted.parent.IsGenesisOfAllele(8, 20)
    assert inserted.parent.parent.IsGenesisOfAllele(7, 12)
...
def test_insert_taxon_internal1(subtrie):
    ...
    assert inserted.parent.IsGenesisOfAllele(5, 11)
...
def test_insert_taxon_internal2(subtrie):
    ...
    assert inserted.taxon_label == "fab"
    assert inserted.parent.IsGenesisOfAllele(6, 21)
    assert anytree_cardinality(subtrie) == previous_node_count + 1
...
def test_insert_taxon_internal3(subtrie):
    ...
    assert inserted.parent.IsGenesisOfAllele(11, 19)
    assert inserted.parent.parent.IsGenesisOfAllele(10, 10)
    assert inserted.parent.parent.parent.IsGenesisOfAllele(6, 21)
