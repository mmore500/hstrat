...
def test_alifestd_prune_extinct_lineages_asexual_polars_cli_csv(tmp_path):
    output_file = str(
        tmp_path
        / "hstrat_alifestd_prune_extinct_lineages_asexual_polars.csv"
    )
    subprocess.run(  # nosec B603
        [
            "python3",
...
def test_alifestd_prune_extinct_lineages_asexual_polars_cli_empty(tmp_path):
    output_file = str(
        tmp_path
        / "hstrat_alifestd_prune_extinct_lineages_asexual_polars.csv"
    )
