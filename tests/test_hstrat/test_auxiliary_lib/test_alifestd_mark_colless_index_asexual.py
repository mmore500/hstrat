...
@pytest.mark.parametrize("mutate", [True, False])
def test_polytomy(mutate: bool):
    """Test a tree with polytomy (more than 2 children).
...
def test_polytomy_imbalanced(mutate: bool):
    ...
    Children of 0: 1 (1 leaf), 2 (1 leaf), 3 (1 leaf)
    Wait, 3 has child 4, so 3 is not a leaf.

    Actually:
    - 1 is a leaf (1 leaf)
    - 2 is a leaf (1 leaf)
    - 3 has child 4, so 3 has 1 leaf (just 4)

    All children of 0 have 1 leaf each, so Colless at root = 0.

    Let's make a more interesting polytomy:
    """
    phylogeny_df = pd.DataFrame(
        {
            "id": [0, 1, 2, 3, 4, 5],
