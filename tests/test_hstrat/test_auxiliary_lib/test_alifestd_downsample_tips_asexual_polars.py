...
def test_alifestd_downsample_tips_asexual_polars(
...
def test_alifestd_downsample_tips_asexual_polars_empty(n_downsample):
...
def test_alifestd_downsample_tips_asexual_polars_no_ancestor_id():
    df = pl.DataFrame(
        {
            "id": [0, 1, 2],
    ...
    with pytest.raises(NotImplementedError):
        alifestd_downsample_tips_asexual_polars(df, 1)


def test_alifestd_downsample_tips_asexual_polars_does_not_mutate():
    ...
    _ = alifestd_downsample_tips_asexual_polars(df, 2, seed=1)

    assert len(df) == original_len
    assert df.columns == original_cols


def test_alifestd_downsample_tips_asexual_polars_simple():
    ...
    result = alifestd_downsample_tips_asexual_polars(df, 1, seed=1)

    assert _count_leaf_nodes_polars(result) == 1
    assert 0 in result["id"].to_list()  # root must be present


def test_alifestd_downsample_tips_asexual_polars_all_tips():
