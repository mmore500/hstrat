...
from hstrat._auxiliary_lib import AnyTreeFastPostOrderIter, consume
...
def test_postorder_iterator(tree_fixture, request):
    tree = request.getfixturevalue(tree_fixture)
    all(
        n1 is n2
        for n1, n2 in zip(
            AnyTreeFastPostOrderIter(tree),
            anytree.PostOrderIter(tree),
        )
    )
...
def test_benchmark(large_tree):
    with ctt.Timer(factor=1000) as t_recursive:
        for __ in range(1):
            consume(anytree.PostOrderIter(large_tree))

    with ctt.Timer(factor=1000) as t_iterative:
        for __ in range(1):
            consume(AnyTreeFastPostOrderIter(large_tree))
